...
__host__ void groestlcoin_cpu_setBlock(int thr_id, void *data)
{
	...
	cudaMemcpyToSymbolAsync(groestlcoin_gpu_msg, msgBlock, 80, 0, cudaMemcpyHostToDevice, streams[thr_id]);

	cudaMemsetAsync(d_resultNonce[thr_id], 0xFF, 2 * sizeof(uint32_t), streams[thr_id]);
}
...
__host__ void groestlcoin_cpu_hash(int thr_id, uint32_t threads, uint32_t startNounce, uint32_t *nounce, uint32_t target)
{
    ...
    groestlcoin_gpu_hash_quad<<<grid, block, 0, streams[thr_id]>>>(threads, startNounce, d_resultNonce[thr_id], thr_id, target);

    CUDA_SAFE_CALL(cudaMemcpyAsync(nounce, d_resultNonce[thr_id], 2 * sizeof(uint32_t), cudaMemcpyDeviceToHost, streams[thr_id])); cudaStreamSynchronize(streams[thr_id]);
}
